...
from rheo.boundaries import create_boundary
...
class MPMSimulator:
    ...
    def build(self, agent, smoke_field, statics, particles):
        ...
        if self.has_agent:
            self.agent_collide_grid = agent.collide_type in ['grid', 'both']
            self.agent_collide_particle = agent.collide_type in ['particle', 'both']
        else:
            self.agent_collide_grid = False
            self.agent_collide_particle = False

        # smoke
        self.smoke_field = smoke_field
        self.has_smoke_field = smoke_field is not None
    ...
    def setup_ckpt_dir(self):
        self.ckpt_dir = os.path.join('/tmp', 'rheo', self.sim_id)
    ...
    def reset_grid_and_grad(self, f: ti.i32):
        ...
        if ti.static(self.dim == 3):
            for env_id, i, j, k in ti.ndrange(self.num_envs, self.res[0], self.res[1], self.res[2]):
                self.grid[env_id, grid_f, i, j, k].v_in = ti.Vector.zero(DTYPE_TI, self.dim)
                self.grid[env_id, grid_f, i, j, k].mass = 0.0
                self.grid[env_id, grid_f, i, j, k].v_out = ti.Vector.zero(DTYPE_TI, self.dim)
            if ti.static(self.enable_grad):
                for env_id, i, j, k in ti.ndrange(self.num_envs, self.res[0], self.res[1], self.res[2]):
                    self.grid.grad[env_id, grid_f, i, j, k].v_in = ti.Vector.zero(DTYPE_TI, self.dim)
                    self.grid.grad[env_id, grid_f, i, j, k].mass = 0.0
                    self.grid.grad[env_id, grid_f, i, j, k].v_out = ti.Vector.zero(DTYPE_TI, self.dim)
        else:  # dim == 2
            for env_id, i, j in ti.ndrange(self.num_envs, self.res[0], self.res[1]):
                self.grid[env_id, grid_f, i, j].v_in = ti.Vector.zero(DTYPE_TI, self.dim)
                self.grid[env_id, grid_f, i, j].mass = 0.0
                self.grid[env_id, grid_f, i, j].v_out = ti.Vector.zero(DTYPE_TI, self.dim)
            if ti.static(self.enable_grad):
                for env_id, i, j in ti.ndrange(self.num_envs, self.res[0], self.res[1]):
                    self.grid.grad[env_id, grid_f, i, j].v_in = ti.Vector.zero(DTYPE_TI, self.dim)
                    self.grid.grad[env_id, grid_f, i, j].mass = 0.0
                    self.grid.grad[env_id, grid_f, i, j].v_out = ti.Vector.zero(DTYPE_TI, self.dim)
            ...
            for env_id, gi, gj, gk in ti.ndrange(self.num_envs, self.res[0], self.res[1], self.res[2]):
                if self.grid[env_id, grid_f, gi, gj, gk].mass > EPS:
                    ...
                    if ti.static(self.has_agent):
                        if ti.static(self.agent_collide_grid):
    ...
    def g2p(self, f: ti.i32):
        ...
        for env_id, p in ti.ndrange(self.num_envs, self.n_particles):
            if self.particles_ng[env_id, pf, p].used:
                ...
                # TODO: Agent collision needs env_id support
                if ti.static(self.has_agent):
                    if ti.static(self.agent_collide_particle):
    ...
    def reset_bodies_and_grad(self):
        for env_id, body_id in ti.ndrange(self.num_envs, self.n_bodies):
            if self.bodies_i[body_id].mat_cls == MAT_RIGID:
                self.bodies[env_id, body_id].COM_t0 = ti.Vector.zero(DTYPE_TI, self.dim)
                self.bodies[env_id, body_id].COM_t1 = ti.Vector.zero(DTYPE_TI, self.dim)
                self.bodies[env_id, body_id].H = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
                self.bodies[env_id, body_id].R = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
                self.bodies[env_id, body_id].U = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
                self.bodies[env_id, body_id].S = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
                self.bodies[env_id, body_id].V = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
        if ti.static(self.enable_grad):
            for env_id, body_id in ti.ndrange(self.num_envs, self.n_bodies):
                if self.bodies_i[body_id].mat_cls == MAT_RIGID:
                    self.bodies.grad[env_id, body_id].COM_t0 = ti.Vector.zero(DTYPE_TI, self.dim)
                    self.bodies.grad[env_id, body_id].COM_t1 = ti.Vector.zero(DTYPE_TI, self.dim)
                    self.bodies.grad[env_id, body_id].H = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
                    self.bodies.grad[env_id, body_id].R = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
                    self.bodies.grad[env_id, body_id].U = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
                    self.bodies.grad[env_id, body_id].S = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
                    self.bodies.grad[env_id, body_id].V = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
    ...
    def reset_grad_till_frame(self, f: ti.i32):
        for env_id, i, j in ti.ndrange(self.num_envs, f, self.n_particles):
            self.particles.grad[env_id, i, j].x = ti.Vector.zero(DTYPE_TI, self.dim)
            self.particles.grad[env_id, i, j].v = ti.Vector.zero(DTYPE_TI, self.dim)
            self.particles.grad[env_id, i, j].C = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
            self.particles.grad[env_id, i, j].F = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
            self.particles.grad[env_id, i, j].F_tmp = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
            self.particles.grad[env_id, i, j].U = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
            self.particles.grad[env_id, i, j].S = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
            self.particles.grad[env_id, i, j].V = ti.Matrix.zero(DTYPE_TI, self.dim, self.dim)
    ...
    def reset_env_grid_kernel(self, env_id: ti.i32, f: ti.i32):
        ...
        for I in ti.grouped(ti.ndrange(*self.res)):
            self.grid[env_id, grid_f, I].v_in = ti.Vector.zero(DTYPE_TI, self.dim)
            self.grid[env_id, grid_f, I].mass = 0.0
            self.grid[env_id, grid_f, I].v_out = ti.Vector.zero(DTYPE_TI, self.dim)

...
def get_raw_mesh_path(file):
    ...
    return os.path.join(get_asset_dir(), 'meshes', 'raw', file)
...
def get_processed_mesh_path(file, file_vis):
    ...
    processed_file_path = os.path.join(get_asset_dir(), 'meshes', 'processed', processed_file)
    return processed_file_path

def get_processed_sdf_path(file, sdf_res):
    assert file.endswith('.obj')
    processed_sdf = f"{file.replace('.obj', '')}-{sdf_res}.sdf"
    processed_sdf_path = os.path.join(get_asset_dir(), 'meshes', 'processed', processed_sdf)
    return processed_sdf_path

def get_voxelized_mesh_path(file, voxelize_res):
    assert file.endswith('.obj')
    return os.path.join(get_asset_dir(), 'meshes', 'voxelized', f"{file.replace('.obj', '')}-{voxelize_res}.vox")

...
import taichi as ti
import numpy as np
from rheo.utils.geom import qmul, w2quat
from rheo.utils.config import make_cls_config
from rheo.utils.misc import *
from rheo.boundaries import create_boundary
from rheo.utils.geom import xyzw_to_wxyz
...
class Effector:
    ...
    def reset_grad_till_frame(self, f):
        for i in range(f):
            self.pos.grad[i] = ti.Vector.zero(DTYPE_TI, 3)
            self.quat.grad[i] = ti.Vector.zero(DTYPE_TI, 4)
            self.v.grad[i] = ti.Vector.zero(DTYPE_TI, 3)
            self.w.grad[i] = ti.Vector.zero(DTYPE_TI, 3)
